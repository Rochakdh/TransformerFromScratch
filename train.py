# Import transformer model and dataset utilities
...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    while True:
        ...
        # Project to vocabulary size and select max probability token
        prob = model.project(out[:, -1])
        _, next_word = torch.max(prob, dim=1)

        # Append predicted word to input
...
# Sentence generators for tokenizer training
...
def get_ds(config):
    ...
    # Logging maximum sentence length
